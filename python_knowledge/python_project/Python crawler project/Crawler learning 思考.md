# 网络爬虫的介绍
## 一、网络爬虫是什么？
爬虫：一段自动抓取网站信息的程序，从互联网上抓取对于我们有价值的信息。  
#按照我的自己个人理解，其实就是收集各个网站信息的程序之名。
***

## 二、网络爬虫的原理
* 发送http请求：爬虫通过http请求从目标网站获取html页面，常用的库有requests
* 解析内容：获取html页面后，爬虫需要解析内容并提取数据，常用的库有Beautifulsoup、lxml、Scrapy等。
* 选取数据：通过定位html元素（如标签、属性、类名等）来提取所需的数据。
* 存储数据：将提取的数据存储到数据库、CSV文件、JSON文件等格式中、以便后续使用或分析。

***
## 三、“爬虫的工具”有关爬虫的第三方库的介绍
*工欲善其事，必先利其器*
实现爬取数据中，我们常常要用到许多第三方库
有requests、beautifulsoup库，还有selenium库
应用框架的有的scrapy、
由于存储数据的Mysql等
此外存储数据后，我们一般还要进行数据分析、数据可视化等工作。又有用到第三方库，我发现，在编程中，对第三方库运用是很重要的一部分。

1. requsets 的介绍
requests是python的第三方库，这里主要用来负责发送网络请求工作。
需要进行额外安装工作，简单的pip 安装即可
    pip install requests

常用用法为get()方法
    requests.get(url,headers=headers)
除了get()方法外，之后还要运用许多有关于它的内容。因为我现在理解琐碎杂乱，这里就不细说。
总之最为常用的方法为get()方法
...
方法：pip安装
    ...
    pip install lxml#推荐使用lxml作为解析器（速度更快，容忍度更高）
2、基本用法
        ...
        try:
            ...
            if content_div:
                ...
                if  response_2:
                    soup_2= BeautifulSoup(response_2.text,'lxml')
                    houxu=soup_2.find("div",id='content').get_text()
                    para_2= houxu.find_all('p')
                    paragraphs_2=[]
                    for line in para_2=[]
                        line=line.strip()
                        paragraphs_2.append(line)
                    cleaned_content_2='/n' .join(paragraphs_2)
                    with open(filename,encoding='utf-8',mode='a') as f:
